...
async function obtenerRecomendacionAleatoria(usuario_id) {
    try {
        const resultado = await pool.query(
            'SELECT s.* FROM series s JOIN usuarios u ON s.categoria = u.categoria_preferida WHERE u.id = $1 AND s.id NOT IN ( SELECT serie_id FROM usuario_serie WHERE usuario_id = $1 ) ORDER BY RANDOM() LIMIT 1;',
            [usuario_id]
        );
        return resultado.rows[0];
    } catch (error) {
        console.error("Error al obtener serie aleatoria por categoría:", error);
        throw error;
    }
}
